...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.cadastrarveiculos.update", [$cadastrarveiculo->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="niv">{{ trans('cruds.cadastrarveiculo.fields.niv') }}</label>
                <input class="form-control {{ $errors->has('niv') ? 'is-invalid' : '' }}" type="text" name="niv" id="niv" value="{{ old('niv', $cadastrarveiculo->niv) }}">
                @if($errors->has('niv'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('placa') ? 'is-invalid' : '' }}" type="text" name="placa" id="placa" value="{{ old('placa', $cadastrarveiculo->placa) }}">
                @if($errors->has('placa'))
                    <div class="invalid-feedback">
                        {{ $errors->first('placa') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('renavam') ? 'is-invalid' : '' }}" type="text" name="renavam" id="renavam" value="{{ old('renavam', $cadastrarveiculo->renavam) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('marca') ? 'is-invalid' : '' }}" type="text" name="marca" id="marca" value="{{ old('marca', $cadastrarveiculo->marca) }}">
                @if($errors->has('marca'))
                    <div class="invalid-feedback">
                        {{ $errors->first('marca') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('descricao') ? 'is-invalid' : '' }}" type="text" name="descricao" id="descricao" value="{{ old('descricao', $cadastrarveiculo->descricao) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('instituicao') ? 'is-invalid' : '' }}" name="instituicao_id" id="instituicao_id">
                    @foreach($instituicaos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('instituicao_id') ? old('instituicao_id') : $cadastrarveiculo->instituicao->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('motorista_responsavels') ? 'is-invalid' : '' }}" name="motorista_responsavels[]" id="motorista_responsavels" multiple>
                    @foreach($motorista_responsavels as $id => $motorista_responsavel)
                        <option value="{{ $id }}" {{ (in_array($id, old('motorista_responsavels', [])) || $cadastrarveiculo->motorista_responsavels->contains($id)) ? 'selected' : '' }}>{{ $motorista_responsavel }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('situacao') ? 'is-invalid' : '' }}" name="situacao" id="situacao">
                    ...
                    @foreach(App\Models\Cadastrarveiculo::SITUACAO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('situacao', $cadastrarveiculo->situacao) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
