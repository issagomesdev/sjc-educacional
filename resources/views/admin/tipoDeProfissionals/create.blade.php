...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tipo-de-profissionals.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\TipoDeProfissional::EDUCADOR_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('educador') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="educador_{{ $key }}" name="educador" value="{{ $key }}" {{ old('educador', '') === (string) $key ? 'checked' : '' }}>
