...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.requisitantes.update", [$requisitante->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="nome">{{ trans('cruds.requisitante.fields.nome') }}</label>
                <input class="form-control {{ $errors->has('nome') ? 'is-invalid' : '' }}" type="text" name="nome" id="nome" value="{{ old('nome', $requisitante->nome) }}" required>
                @if($errors->has('nome'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('descricao') ? 'is-invalid' : '' }}" name="descricao" id="descricao">{{ old('descricao', $requisitante->descricao) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cep') ? 'is-invalid' : '' }}" type="text" name="cep" id="cep" value="{{ old('cep', $requisitante->cep) }}">
                @if($errors->has('cep'))
                    <div class="invalid-feedback">
                        {{ $errors->first('cep') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" name="estado" id="estado">
                    ...
                    @foreach(App\Models\Requisitante::ESTADO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('estado', $requisitante->estado) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cidade') ? 'is-invalid' : '' }}" type="text" name="cidade" id="cidade" value="{{ old('cidade', $requisitante->cidade) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bairro') ? 'is-invalid' : '' }}" type="text" name="bairro" id="bairro" value="{{ old('bairro', $requisitante->bairro) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('endereco') ? 'is-invalid' : '' }}" type="text" name="endereco" id="endereco" value="{{ old('endereco', $requisitante->endereco) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('numero') ? 'is-invalid' : '' }}" type="text" name="numero" id="numero" value="{{ old('numero', $requisitante->numero) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('estoques') ? 'is-invalid' : '' }}" name="estoques[]" id="estoques" multiple>
                    @foreach($estoques as $id => $estoque)
                        <option value="{{ $id }}" {{ (in_array($id, old('estoques', [])) || $requisitante->estoques->contains($id)) ? 'selected' : '' }}>{{ $estoque }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('situacao') ? 'is-invalid' : '' }}" name="situacao" id="situacao">
                    ...
                    @foreach(App\Models\Requisitante::SITUACAO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('situacao', $requisitante->situacao) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
