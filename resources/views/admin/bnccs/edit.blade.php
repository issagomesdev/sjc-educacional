...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bnccs.update", [$bncc->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="codigo">Código</label>
                <input class="form-control {{ $errors->has('codigo') ? 'is-invalid' : '' }}" type="text" name="codigo" id="codigo" value="{{ old('codigo', $bncc->codigo) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('objetivo_habilidade') ? 'is-invalid' : '' }}" name="objetivo_habilidade" id="objetivo_habilidade">{{ old('objetivo_habilidade', $bncc->objetivo_habilidade) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('nivel_de_ensino') ? 'is-invalid' : '' }}" name="nivel_de_ensino" id="nivel_de_ensino">
                    ...
                    @foreach(App\Models\Bncc::NIVEL_DE_ENSINO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('nivel_de_ensino', $bncc->nivel_de_ensino) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('aprendizagem_desenvolvimento') ? 'is-invalid' : '' }}" name="aprendizagem_desenvolvimento" id="aprendizagem_desenvolvimento">{{ old('aprendizagem_desenvolvimento', $bncc->aprendizagem_desenvolvimento) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('disciplina') ? 'is-invalid' : '' }}" name="disciplina_id" id="disciplina_id">
                    @foreach($disciplinas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('disciplina_id') ? old('disciplina_id') : $bncc->disciplina->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unidade_tematica') ? 'is-invalid' : '' }}" type="text" name="unidade_tematica" id="unidade_tematica" value="{{ old('unidade_tematica', $bncc->unidade_tematica) }}">
