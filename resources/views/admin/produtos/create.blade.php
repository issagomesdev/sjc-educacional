    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.produtos.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unidade') ? 'is-invalid' : '' }}" type="number" name="unidade" id="unidade" value="{{ old('unidade', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categorias') ? 'is-invalid' : '' }}" name="categorias[]" id="categorias" multiple>
                    @foreach($categorias as $id => $categoria)
                        <option value="{{ $id }}" {{ in_array($id, old('categorias', [])) ? 'selected' : '' }}>{{ $categoria }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('descricao') ? 'is-invalid' : '' }}" type="text" name="descricao" id="descricao" placeholder="Informe algo sobre esse registro caso acredite ser necessário…" value="{{ old('descricao', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('consumivel') ? 'is-invalid' : '' }}" name="consumivel" id="consumivel">
                    ...
                    @foreach(App\Models\Produto::CONSUMIVEL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('consumivel', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('estoque_minimo') ? 'is-invalid' : '' }}" type="number" name="estoque_minimo" id="estoque_minimo" value="{{ old('estoque_minimo', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('estoque_maximo') ? 'is-invalid' : '' }}" type="number" name="estoque_maximo" id="estoque_maximo" value="{{ old('estoque_maximo', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('localizacao') ? 'is-invalid' : '' }}" type="text" name="localizacao" id="localizacao" value="{{ old('localizacao', '') }}">
