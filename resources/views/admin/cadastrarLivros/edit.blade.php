...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.cadastrar-livros.update", [$cadastrarLivro->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="titulo">{{ trans('cruds.cadastrarLivro.fields.titulo') }}</label>
                <input class="form-control {{ $errors->has('titulo') ? 'is-invalid' : '' }}" type="text" name="titulo" id="titulo" value="{{ old('titulo', $cadastrarLivro->titulo) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('autor') ? 'is-invalid' : '' }}" type="text" name="autor" id="autor" value="{{ old('autor', $cadastrarLivro->autor) }}">
                @if($errors->has('autor'))
                    <div class="invalid-feedback">
                        {{ $errors->first('autor') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('idioma') ? 'is-invalid' : '' }}" type="text" name="idioma" id="idioma" value="{{ old('idioma', $cadastrarLivro->idioma) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('biblioteca') ? 'is-invalid' : '' }}" name="biblioteca_id" id="biblioteca_id">
                    @foreach($bibliotecas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('biblioteca_id') ? old('biblioteca_id') : $cadastrarLivro->biblioteca->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ano') ? 'is-invalid' : '' }}" type="number" name="ano" id="ano" value="{{ old('ano', $cadastrarLivro->ano) }}" step="1">
                @if($errors->has('ano'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ano') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('editora') ? 'is-invalid' : '' }}" type="text" name="editora" id="editora" value="{{ old('editora', $cadastrarLivro->editora) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('genero') ? 'is-invalid' : '' }}" name="genero" id="genero">
                    ...
                    @foreach(App\Models\CadastrarLivro::GENERO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('genero', $cadastrarLivro->genero) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('assunto') ? 'is-invalid' : '' }}" type="text" name="assunto" id="assunto" value="{{ old('assunto', $cadastrarLivro->assunto) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('materias_relacionadas') ? 'is-invalid' : '' }}" name="materias_relacionadas[]" id="materias_relacionadas" multiple>
                    @foreach($materias_relacionadas as $id => $materias_relacionada)
                        <option value="{{ $id }}" {{ (in_array($id, old('materias_relacionadas', [])) || $cadastrarLivro->materias_relacionadas->contains($id)) ? 'selected' : '' }}>{{ $materias_relacionada }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('exemplares_existentes') ? 'is-invalid' : '' }}" type="text" name="exemplares_existentes" id="exemplares_existentes" value="{{ old('exemplares_existentes', $cadastrarLivro->exemplares_existentes) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('isbn') ? 'is-invalid' : '' }}" type="text" name="isbn" id="isbn" value="{{ old('isbn', $cadastrarLivro->isbn) }}" step="1">
                @if($errors->has('isbn'))
                    <div class="invalid-feedback">
                        {{ $errors->first('isbn') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cdd') ? 'is-invalid' : '' }}" type="number" name="cdd" id="cdd" value="{{ old('cdd', $cadastrarLivro->cdd) }}" step="1">
                @if($errors->has('cdd'))
                    <div class="invalid-feedback">
                        {{ $errors->first('cdd') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\CadastrarLivro::SELECIONE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('selecione') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="selecione_{{ $key }}" name="selecione" value="{{ $key }}" {{ old('selecione', $cadastrarLivro->selecione) === (string) $key ? 'checked' : '' }}>
