...
<form method="GET" action="{{ route("admin.relatorios-do-almoxarifados.estoques") }}">
...
<option value="all"> Todos os estoques </option>
 ...
 <option value="{{ $estoque->id }}" {{ old('escola', $estoque->id) == $request_estoque ? 'selected' : '' }}>{{ $estoque->titulo }}</option>
 @endforeach
 </select>
...
<div class="card">
    <div class="card-header">
        Relatórios de Itens no Estoque
    </div>
 </div>

<!-- itens no estoque -->
 <div class="grapic">
   <table id="example" class=" table table-bordered table-striped table-hover datatable datatable-estoque">
     <thead>
         <tr>
       ...
       <tbody>
         ...
         <td></td>
             ...
             <td>
                {{ App\Models\Produto::SITUACAO_SELECT[$produto->situacao] ?? 'Disponível' }}
             </td>
             <td>
...
<div class="card">
    <div class="card-header">
        Relatórios de Itens abaixo do estoque minimo
    </div>
 </div>

<div class="grapic">
  <table id="example" class=" table table-bordered table-striped table-hover datatable datatable-min">
    <thead>
        <tr>
          <th class="noExport"> </th>
          <th class="noSorting"> </th>
      ...
      <tbody>
        <?php  $soma_quant = []; foreach($produtos_no_estoque as $key=>$value){ if (isset($soma_quant[$value["produto_id"]]))
         $soma_quant[$value["produto_id"]] += $value["quantidade"];
         else $soma_quant[$value["produto_id"]] = $value["quantidade"]; }
        ...
        <?php foreach($produtos_no_estoque as $produto_no_estoque) {

        if(!in_array($produto->id, $produtos_array)) {
        ...
        elseif ($produto_no_estoque['produto_id'] == $produto->id) {

        $mins = $produto_no_estoque['quantidade']; }

      } ?>

      @if($mins < $produto->estoque_minimo)
        ...
        <td></td>
            ...
            <td>
              @foreach($produtos_no_estoque as $produto_no_estoque) @if($produto_no_estoque['produto_id'] == $produto->id)
              {{ $produto_no_estoque['quantidade']; }} @endif @endforeach
               @if(!in_array($produto->id, $produtos_array)) 0 @endif
...
<script>
  ...
  let table = $('.datatable-min:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
