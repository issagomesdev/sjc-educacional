...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.requisicos.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                <label class="requitantes" for="requisitantes_id">{{ trans('cruds.requisico.fields.requisitantes') }}</label>
                <select class="form-control select2 {{ $errors->has('requisitantes') ? 'is-invalid' : '' }}" name="requisitantes_id" id="requisitantes_id" required>
                    @foreach($requisitantes as $id => $entry)
                        <option value="{{ $id }}" {{ old('requisitantes_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
