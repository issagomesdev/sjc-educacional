...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.conteudos-curriculares.store") }}" enctype="multipart/form-data">
            @csrf
                ...
                <select class="form-control select2" name="bncc_id" id="bncc_id">
                  <option value="">Selecione por favor</option>
                    @foreach($bnccs as $bncc)
                        <option value="{{ $bncc->id }}" {{ old('bncc_id') == $bncc->id ? 'selected' : '' }}> {{ $bncc->codigo }} • {{$bncc->nivel_de_ensino}} • {{ $bncc->disciplina->nome_da_materia ?? '' }}</option>
                    @endforeach
                </select>
                <span class="help-block"> </span>
            </div>
            <div class="form-group" id="cdp" style="display:none;">
                ...
                <select class="form-control select2" name="cdp_id" id="cdp_id">
                  <option value="">Selecione por favor</option>
                    @foreach($cdps as $cdp)
                        <option value="{{ $cdp->id }}" {{ old('cdp_id') == $cdp->id ? 'selected' : '' }}>{{ $cdp->codigo }} • {{$cdp->nivel_de_ensino}} • {{ $cdp->disciplina->nome_da_materia ?? '' }}</option>
                    @endforeach
                </select>
                <span class="help-block"> </span>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" name="serie[]" id="serie" multiple>
                    <option value="Creche I" {{ in_array('Creche I', old('turmas', [])) ? 'selected' : '' }}> Creche - 0 a 1 ano e 6 meses </option>
                    <option value="Creche II" {{ in_array('Creche II', old('turmas', [])) ? 'selected' : '' }}> Creche - 1 ano e 7 meses a 3 anos e 11 meses </option>
                    <option value="Pré-escolar" {{ in_array('Pré-escolar', old('turmas', [])) ? 'selected' : '' }}> Pré-escolar - 4 a 5 anos </option>
                    <option value="1º ano" {{ in_array('1º ano', old('turmas', [])) ? 'selected' : '' }}> 1º ano - Fundamental I </option>
                    <option value="2º ano" {{ in_array('2º ano', old('turmas', [])) ? 'selected' : '' }}> 2º ano - Fundamental I </option>
                    <option value="3º ano" {{ in_array('3º ano', old('turmas', [])) ? 'selected' : '' }}> 3º ano - Fundamental I </option>
                    <option value="4º ano" {{ in_array('4º ano', old('turmas', [])) ? 'selected' : '' }}> 4º ano - Fundamental I </option>
                    <option value="5º ano" {{ in_array('5º ano', old('turmas', [])) ? 'selected' : '' }}> 5º ano - Fundamental I </option>
                    <option value="6º ano" {{ in_array('6º ano', old('turmas', [])) ? 'selected' : '' }}> 6º ano - Fundamental II </option>
                    <option value="7º ano" {{ in_array('7º ano', old('turmas', [])) ? 'selected' : '' }}> 7º ano - Fundamental II </option>
                    <option value="8º ano" {{ in_array('8º ano', old('turmas', [])) ? 'selected' : '' }}> 8º ano - Fundamental II </option>
                    <option value="9º ano" {{ in_array('9º ano', old('turmas', [])) ? 'selected' : '' }}> 9º ano - Fundamental II </option>
                    <option value="1º ano(EJA)" {{ in_array('1º ano(EJA)', old('turmas', [])) ? 'selected' : '' }}> 1º ano - EJA </option>
                    <option value="2º ano(EJA)" {{ in_array('2º ano(EJA)', old('turmas', [])) ? 'selected' : '' }}> 2º ano - EJA </option>
                    <option value="3º ano(EJA)" {{ in_array('3º ano(EJA)', old('turmas', [])) ? 'selected' : '' }}> 3º ano - EJA </option>
                </select>
                <span class="help-block"> </span>
            </div>
            <div class="form-group" id="disciplina" style="display:none;">
                ...
                <select class="form-control select2" name="disciplina_id" id="disciplina_id" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('bimestres') ? 'is-invalid' : '' }}" name="bimestres" id="bimestres" required>
                    ...
                    @foreach(App\Models\ConteudosCurriculare::BIMESTRES_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('bimestres', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('campo_eixo') ? 'is-invalid' : '' }}" name="campo_eixo" id="campo_eixo">
                    ...
                    @foreach(App\Models\ConteudosCurriculare::CAMPO_EIXO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('campo_eixo', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('conteudo') ? 'is-invalid' : '' }}" type="text" name="conteudo" id="conteudo" value="{{ old('conteudo', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('analises_linguisticas') ? 'is-invalid' : '' }}" type="text" name="analises_linguisticas" id="analises_linguisticas" value="{{ old('analises_linguisticas', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('recurso_didatico') ? 'is-invalid' : '' }}" name="recurso_didatico" id="recurso_didatico">
                    ...
                    @foreach(App\Models\ConteudosCurriculare::RECURSO_DIDATICO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('recurso_didatico', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('situacao_didatica') ? 'is-invalid' : '' }}" name="situacao_didatica" id="situacao_didatica">
                    ...
                    @foreach(App\Models\ConteudosCurriculare::SITUACAO_DIDATICA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('situacao_didatica', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('conteudos_trabalhados') ? 'is-invalid' : '' }}" name="conteudos_trabalhados" id="conteudos_trabalhados">{!! old('conteudos_trabalhados') !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('complementos_de_conteudo') ? 'is-invalid' : '' }}" name="complementos_de_conteudo" id="complementos_de_conteudo">{!! old('complementos_de_conteudo') !!}</textarea>
...
$("#bncc_x_cdp").on('change', function() {
const bncc_x_cdp = this.value;
if (bncc_x_cdp == 'BNCC') {
$("#bncc_id").prop('required', true);
} else {
$("#bncc_id").prop('required', false);
}

if (bncc_x_cdp == 'CDP') {
$("#cdp_id").prop('required', true);
} else {
$("#cdp_id").prop('required', false);
}
});
